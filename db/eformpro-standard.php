...
$userid=$_SESSION['userid'];
    ...
    if (empty($gfn)) {//echo "jaza fomu ";
        header("Location: ../eform-standard.php?error=Complite Form");
        exit();
    }
    else{ 
        ...
        if (mysqli_num_rows($result) > 0) { //echo "yupo ipo";
            header("Location: ../eform-standard.php?error= Groom Details Exist");
            exit();
        }else {
           ...
           if ($result2) { //echo "done";
            header("Location: ../event-standard.php?success=Your Event has been created successfully");
            exit();
          }else {//echo "bado";
               header("Location: ../event-standard.php?error=Failed&$user_data");
               exit();
          }
       }
       }}
